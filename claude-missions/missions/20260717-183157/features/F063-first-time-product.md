# F063: First-time product entry from scan flow (label + barcode)

**Milestone:** M7 — Vision
**Estimated worker time:** 30 minutes
**Depends on:** F062, F032

## Assertion IDs covered
- AS-105 (label-created product with GTIN available to all, neprovereno)

## Draft scope
- Unknown-barcode flow upgraded: prompt to photograph label (and barcode if not already scanned)
- Extracted values + GTIN → confirmation form → save shared food (verified=false) → portion picker → log
- Completes the crowdsourced growth loop: first user pays one vision call, everyone else scans free

## Files (approximate)
scan flow wiring, src/app/(app)/dodaj/novi-proizvod/
...
## Clarified implementation (from clarifications/F063-clarification.md)
