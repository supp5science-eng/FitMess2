# F061: Photo capture/upload + private storage

**Milestone:** M7 — Vision
**Estimated worker time:** 45 minutes
**Depends on:** F005, F002

## Assertion IDs covered
- AS-111 (users cannot access other users' photos)

## Draft scope
- Camera/file input component (mobile camera capture attr), client-side downscale before upload
- Supabase Storage private bucket with per-user path RLS policies; signed URLs for display
- Shared by label and meal flows

## Files (approximate)
src/components/photo/PhotoCapture.tsx, storage policies migration, src/lib/storage.ts
...
## Clarified implementation (from clarifications/F061-clarification.md)
