# F025: Log entry creation — grams + common units
...
- AS-041 (log by grams, macros computed), AS-042 (log by common units via gram weights)

## Draft scope
- Portion picker sheet: gram input + unit chips (parče/kašika/šolja/komad) where defined
- Server action creating log row with computed kcal/macros snapshot
- Success feedback + immediate return to home

## Files (approximate)
src/components/food/PortionPicker.tsx, src/app/api/logs actions, src/lib/food/portions.ts (+ tests)
