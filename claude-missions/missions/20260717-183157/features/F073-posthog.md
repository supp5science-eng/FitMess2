# F073: PostHog EU — consent gate + core events
...
- AS-120 (core events captured), AS-121 (EU endpoint only), AS-123 (tracking only after consent)

## Draft scope
- posthog-js init with api_host https://eu.i.posthog.com, opt-in mode (no tracking pre-consent)
- Consent banner/toggle in Serbian; persisted choice
- Events: signup, onboarding_completed, meal_logged (method), agent_used, photo_logged

## Files (approximate)
src/lib/analytics.ts, consent component, event calls at key points

## Notes for clarification
- MCP at run: PostHog MCP if registered


---

## Clarified implementation (from clarifications/F073-clarification.md)
