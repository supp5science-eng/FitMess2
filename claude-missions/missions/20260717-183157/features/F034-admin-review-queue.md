# F034: Admin review queue — verify/remove foods

**Milestone:** M4 — Barcode & Admin
**Estimated worker time:** 45 minutes
**Depends on:** F033, F020

## Assertion IDs covered
- AS-060 (unverified queue), AS-062 (mark verified), AS-063 (remove food from search/scan)

## Draft scope
- Queue list of verified=false foods, newest first, with submitter + label photo when present
- Verify action; remove/pull action (soft-delete flag so existing logs keep their snapshot)

## Files (approximate)
src/app/admin/hrana/page.tsx, admin food actions
...
## Clarified implementation (from clarifications/F034-clarification.md)
