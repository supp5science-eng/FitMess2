# F030: Barcode scanner component (client-side EAN-13)
...
- AS-052 (client-side EAN-13 decode, no API), AS-058 (permission-denied fallback)

## Draft scope
- barcode-detector package (ZXing WASM polyfill of BarcodeDetector API), camera stream via getUserMedia
- Scanner overlay UI with viewfinder; EAN-13 (+ EAN-8) formats
- Permission denied / no camera: Serbian message + link to manual search

## Files (approximate)
src/components/scan/BarcodeScanner.tsx, src/app/(app)/dodaj/skener/page.tsx
...
## Clarified implementation (from clarifications/F030-clarification.md)
