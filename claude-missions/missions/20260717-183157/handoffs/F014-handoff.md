...
AS-086: PASS (foundational scope only) — AS-086's own text names a "redistribution" function, which is F053's feature, not this one; F014 doesn't implement redistribution. What F014 does deliver is the underlying principle the clarification cites AS-086 for ("All arithmetic here is deterministic; LLM never computes these numbers"): every budget number (BMR, TDEE, daily/weekly targets, macros, goal-adjustment) is produced by a pure, side-effect-free, exhaustively unit-tested function in `src/lib/budget/engine.ts`, with zero LLM involvement, establishing the exact pattern (`src/lib/budget/*` + colocated `*.test.ts`, per tech-decisions.md's "Money-math rule") that F053's redistribution function must follow. Full closure of AS-086 itself (a deterministic, unit-tested *redistribution* function) remains F053's responsibility — see Out-of-scope work needed.
...
- F053 (redistribution engine) is the feature that must fully close AS-086: it needs to implement the actual deterministic, unit-tested redistribution function (overshoot spread across the next 2–3 days, capped at 200 kcal/day per AS-087) following the same pure-function-in-`src/lib/budget/`-with-colocated-tests pattern this feature established. Not implemented here — F014's scope was BMR/TDEE/macros/goal-adjustment only, per its "Assertion IDs covered" list (AS-021–AS-026, AS-030).
