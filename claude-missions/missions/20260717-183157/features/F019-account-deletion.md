...
- AS-015 (self-serve delete removes personal data), AS-016 (credentials stop authenticating)

## Draft scope
- Confirmation dialog (type-to-confirm) in Serbian
- Server action: delete user rows (cascade), delete auth user via secret key, sign out
- Shared foods contributed by the user remain, submitter anonymized

## Files (approximate)
src/app/api/account/delete/route.ts, profile settings UI
